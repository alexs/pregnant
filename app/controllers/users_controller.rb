...
before_filter :require_admin, :only => [:index, :new]
  ...
  def index
    ...
    if current_user.group_id == 3
       ...
       @users = User.searcher(params[:search_word], nil, :order => 'name ASC').paginate :per_page => 20, :page => params[:page]
    ...
    respond_to do |format|
      format.js { render :action => 'search_user'}
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @user.save
        format.js {render :action => 'create.js.rjs', :notice => 'Cuenta registrada, ahora podra iniciar sesion con este usuario.'}
      else
